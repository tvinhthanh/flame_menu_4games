...
enum GameState { loading, intro, waitingForTitle, interactive, error }

class GopControlGame extends FlameGame {
  ...
  Future<void> onLoad() async {
    try {
      ...
      _smCtrl = rive.StateMachineController.fromArtboard(
        _artboard,
        _stateMachineName,
      );
      if (_smCtrl != null) {
  ...
  Future<void> _playIntroSequence() async {
    final animations = ["Nhanvat_In", "HoaSen_In", "LinhHon_In", "Background"];
  ...
  Future<void> _playOneShot(String name) {
    ...
    controller = OneShotAnimation(
      ...
      onStop: () {
        ...
        _artboard.removeController(
          controller,
        ); // OK vì controller đã khai báo trước
        completer.complete();
      },
    );
